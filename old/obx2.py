...
try:
    ...
    for flag in argv[1:]:
        if flag in ["-f", "--fancy"]:
            config["fancy"] = True
        elif flag in ["-iobx2", "--ignoreobjective"]:
            config["ignoreObjectiveException"] = True
        elif flag in ["-ipy", "--ignorepython"]:
            config["ignorePythonException"] = True
        elif flag in ["-nf", "--nofancy"]:
            config["fancy"] = False
        elif flag in ["-nq", "--noquiet"]:
            config["fancy"] = False
        elif flag in ["-niobx2", "--noignoreobjective"]:
            config["ignoreObjectiveException"] = False
        elif flag in ["-nipy", "--noignorepython"]:
            config["ignorePythonException"] = False
        elif flag in ["-q", "--q"]:
            config["quiet"] = True
        else:
...
# Exceptions
    ...
    sys.exit(0 if config.get("quiet") == True or config.get("ignoreObjectiveException") else repr(InterpreterException))
except Exception as PythonException:
    sys.exit(0 if config.get("quiet") == True or config.get("ignorePythonException") == True else repr(PythonException))