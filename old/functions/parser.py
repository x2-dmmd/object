...
def parseFields(lines: list) -> list:
    ...
    for line in lines:
        ...
        if not line:
            continue
        if line.startswith(("class ", "if ", "repeat ", "sect ", "while ")) or line == "try":
            subfield = []
...
def parseTokens(line: str) -> list:
    ...
    if token:
        tokens.append(token)
    for token in tokens:
        if not token.startswith(("\"", "'", "(")) and not tools.isNumber(token) and (not token[0].isalpha() or any([character not in tools.characters for character in token])):
